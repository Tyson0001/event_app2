...
class _EventDetailsPageState extends State<EventDetailsPage> {
  ...
  void initState() {
    super.initState();
    imagesUrl =
        'https://gatherhub-r7yr.onrender.com/user/conference/${widget.event['conferenceCode']}/eventCard/images';
    _loadNotificationCount();
    _loadSavedImages();
  }
  ...
  Future<int> fetchNotificationCount() async {
    await Future.delayed(const Duration(seconds: 1));
    return 5; // Replace with your actual notification fetching logic
  }
  ...
  Future<void> _reloadImages() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _showSnackBar('Failed to load images. Please try again.');
      }
    } catch (e) {
      print('Error fetching event images: $e');
      _showSnackBar('Error loading images. Please check your connection.');
    } finally {
